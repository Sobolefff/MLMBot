...
- Cloud Bot API Telegram не отдаёт боту файлы тяжелее 20 МБ (ограничение самого Telegram, не бота) — для больших PDF (с большим количеством фото) нужен либо облегчённый экспорт каталога, либо self-hosted Bot API сервер.
